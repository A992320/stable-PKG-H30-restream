...
if (!$allowed) {
    if (subsEnsureSchema() && subsProtectionOn()) {
        ...
        $allowed = $u && subsUserStatus($u)['active'] && subsValidateUserSession($__siteUid, $__siteSid);
    } else {
...
rxJson([
    ...
    'ping'    => 'restream.php?' . ($kind === 'e' ? 'ep=' : 'ch=') . $id . ($compatVideo ? '&compat=1' : '') . '&ping=1',
    'compat'  => $compatVideo,
]);
