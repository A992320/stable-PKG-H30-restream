...
.shsx-hero.shsx-empty{ /* display:none removed to show skeleton */ }
...
@keyframes shsxSpinPulse{0%{transform:rotate(0deg);opacity:.8}50%{opacity:.4}100%{transform:rotate(360deg);opacity:.8}}
...
(function(){
  ...
  function mount(){
    ...
    hero.innerHTML=
      '<div class="shsx-hero-skel" style="position:absolute;inset:0;z-index:10;display:flex;align-items:center;justify-content:center;background:#0b0b0b"><div style="width:34px;height:34px;border:3px solid rgba(229,9,20,.2);border-top-color:#e50914;border-radius:50%;animation:shsxSpinPulse 1s linear infinite"></div></div>'+
  ...
  /* ── جلب أعمال عشوائية للبانر للحفاظ على تنوع حقيقي ── */
  async function fetchHeroItems(){
    if(!window.App || !App.cats || !App.cats.length) return false;
    
    var validCats = App.cats.filter(function(c) {
      return (parseInt(c.series_count||0) > 0) || (parseInt(c.channel_count||0) > 0);
    });
    if(!validCats.length) return false;
    
    var chosenCats = shuffle(validCats).slice(0, 7);
    var promises = chosenCats.map(function(c) {
      var isVod = parseInt(c.series_count||0) > 0;
      var act = isVod ? 'series' : 'channels';
      return fetch('api.php?action='+act+'&category_id='+c.id+'&limit=6')
        .then(function(r){ return r.json(); })
        .then(function(d){ 
           var items = isVod ? d.series : d.channels;
           if(items && items.length) {
             ...
             if(pool.length){
               ...
               it.globalType = isVod ? 'series' : 'channel';
               it.cat_name = c.name;
               return it;
             }
           }
    ...
    var finalItems = results.filter(function(x) { return x && x.name; });
    
    if(finalItems.length > 0) {
       finalItems.forEach(function(it, idx) {
         if(idx === 0) it._tag = 'الأكثر مشاهدة';
         else if(idx === 1) it._tag = 'أُضيف حديثاً';
         else it._tag = 'مقترح لك';
       });
       HERO.items = finalItems;
       return true;
    }
    return false;
  }
  ...
  function waitAndBuild(){
    ...
    var iv=setInterval(async function(){
      ...
      if(window.App && App.cats && App.cats.length > 0) {
        clearInterval(iv);
        var success = await fetchHeroItems();
        if(success) build();
        else {
          // If fetching fails, hide the banner gracefully
