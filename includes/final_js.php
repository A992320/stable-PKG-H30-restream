...
function normalizeOmdbKey(value) {
    const raw = String(value || '').trim();
    if (!raw) return '';

    const match = raw.match(/(?:^|[?&])apikey=([^&#\s]+)/i);
    if (!match || !match[1]) return raw.replace(/^["']|["']$/g, '');

    try { return decodeURIComponent(match[1]).trim(); }
    catch (e) { return match[1].trim(); }
}
async function testApiOmdb() {
    const input = document.getElementById('api_omdb_key');
    const key = normalizeOmdbKey(input.value);
    const st = document.getElementById('status_omdb');
    if (key && key !== input.value.trim()) input.value = key;
    if(!key) {
        st.innerHTML = '<span style="color:var(--red)"><i class="fas fa-times-circle"></i> ' + window.lang.please_enter_key + '</span>';
        return;
    }
    st.innerHTML = '<span style="color:var(--t3)"><i class="fas fa-spinner fa-spin"></i> ' + window.lang.scanning + '</span>';
    try {
        const r = await fetch(`https://www.omdbapi.com/?i=tt3896198&apikey=${encodeURIComponent(key)}`);
