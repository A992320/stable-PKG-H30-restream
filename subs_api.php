...
case 'plan_save': {
    ...
    if (!preg_match('/^[\\p{L}\\p{N}_-]{2,32}$/u', $code)) sErr('bad_code');
...
case 'subs_list': {
    ...
    $st = db()->prepare(
        "SELECT u.id,u.username,u.email,u.is_active,u.plan_id,u.sub_start,u.sub_end,
                u.activated_via,u.last_login,u.last_ip,u.notes,u.created_at,u.max_devices,
         ...
         LEFT JOIN (
             ...
             WHERE revoked_at IS NULL AND last_seen >= DATE_SUB(NOW(), INTERVAL 2 HOUR)
             GROUP BY user_id
...
case 'sub_create': {
    ...
    if (!$r['ok']) sErr($r['error']);
    $maxDevices = max(1, min(10, (int)($_POST['max_devices'] ?? subsDefaultDeviceLimit())));
    $deviceUpdate = db()->prepare("UPDATE site_users SET max_devices=? WHERE id=?");
    $deviceUpdate->execute([$maxDevices, (int)$r['id']]);
...
case 'sub_update': {
    ...
    $st = db()->prepare("UPDATE site_users SET email=?, notes=?, max_devices=? WHERE id=?");
    $st->execute([$email ?: null, $notes ?: null, $maxDevices, $id]);
    subsPruneUserSessions($id);
    $currentDevices = db()->prepare("SELECT COUNT(*) FROM site_user_sessions WHERE user_id=? AND revoked_at IS NULL AND last_seen >= DATE_SUB(NOW(), INTERVAL 2 HOUR)");
    $currentDevices->execute([$id]);
    if ((int)$currentDevices->fetchColumn() > $maxDevices) subsClearUserSessions($id);
...
case 'sub_delete': {
    $id = (int)($_POST['id'] ?? 0);
    if ($id < 1) sErr('bad_id');
    // جلسات الدخول لا تحمل مفتاحاً أجنبياً كي تبقى الترقية متوافقة مع التركيبات القديمة.
    subsClearUserSessions($id);
...
case 'sub_sessions_clear': {
    $id = (int)($_POST['id'] ?? 0);
    if ($id < 1 || !subsUserById($id)) sErr('user_not_found', 404);
    if (!subsClearUserSessions($id)) sErr('server', 500);
    if (function_exists('logTo')) logTo('admin', "تسجيل خروج كل أجهزة المشترك #$id بواسطة $admin");
    sOk();
    break;
}
...
case 'settings_get':
    sOk(['settings'=>[
        ...
        'default_max_devices' => subsSetting('default_max_devices','1'),
    ]]);
    break;
